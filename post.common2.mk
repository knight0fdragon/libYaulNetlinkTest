...
$(SH_PROGRAM).iso: $(SH_BUILD_PATH)/$(SH_PROGRAM).bin $(SH_BUILD_PATH)/IP.BIN
	mv -f $(SH_BUILD_PATH)/IP.BIN $(IMAGE_DIRECTORY)/SAMPLEIP.BIN
	...
	$(ECHO)$(YAUL_INSTALL_ROOT)/share/wrap-error $(YAUL_INSTALL_ROOT)/bin/make-iso $(IMAGE_DIRECTORY) XBOS_IP.BIN bin/$(SH_PROGRAM)
...
$(SH_PROGRAM).cue: $(SH_PROGRAM).iso
	@printf -- "$(V_BEGIN_YELLOW)$@$(V_END)\n"
	$(ECHO)$(YAUL_INSTALL_ROOT)/share/wrap-error $(YAUL_INSTALL_ROOT)/bin/make-cue "bin/$(SH_PROGRAM).iso"
